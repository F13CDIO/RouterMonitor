...
public class DataPackageDAO implements IDataPackageDAO
{
	...
	public JSONObject get1HourTraffic(Date date, String host) throws SQLException 
	{
		...
		java.sql.Timestamp mySqlTimestampTo = new java.sql.Timestamp(date.getTime());
		ResultSet mySqlOutput = MySQLConnector.execQuery("SELECT timestamp, COUNT(*) AS count FROM dataPackages WHERE timestamp BETWEEN ('"+mySqlTimestampTo+"' - INTERVAL 1 HOUR) and '"+mySqlTimestampTo+"' "+host+" GROUP BY UNIX_TIMESTAMP(timestamp) DIV 60 LIMIT 60");
		return parseResultsetToJSONObject("traffic", mySqlOutput);
	}
	...
	public JSONObject get1DayTraffic(Date date, String host) throws SQLException 
	{
		...
		java.sql.Timestamp mySqlTimestampTo = new java.sql.Timestamp(date.getTime());
		ResultSet mySqlOutput = MySQLConnector.execQuery("SELECT timestamp, COUNT(*) AS count FROM dataPackages WHERE timestamp BETWEEN ('"+mySqlTimestampTo+"' - INTERVAL 1 DAY) and '"+mySqlTimestampTo+"' "+host+" GROUP BY UNIX_TIMESTAMP(timestamp) DIV 3600 LIMIT 24");
		return parseResultsetToJSONObject("traffic", mySqlOutput);
	}
	...
	public JSONObject get1MonthTraffic(Date date, String host) throws SQLException 
	{
		...
		java.sql.Timestamp mySqlTimestampTo = new java.sql.Timestamp(date.getTime());
		ResultSet mySqlOutput = MySQLConnector.execQuery("SELECT timestamp, COUNT(*) AS count FROM dataPackages WHERE timestamp BETWEEN ('"+mySqlTimestampTo+"' - INTERVAL 1 MONTH) and '"+mySqlTimestampTo+"' "+host+" GROUP BY UNIX_TIMESTAMP(timestamp) DIV 86400 LIMIT 31");
		return parseResultsetToJSONObject("traffic", mySqlOutput);
	}
